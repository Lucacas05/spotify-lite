...
- [x] `LibrespotEngine`: launch the external binary with `Process`:
  - `librespot --name "SpotifyLite" --backend rodio --zeroconf-backend dns-sd --device-type computer --bitrate 320 --system-cache <Application Support>/SpotifyLite/librespot/accounts/<spotify-user-id>` (plus `--enable-oauth` on first run only).

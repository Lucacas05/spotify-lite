...
- Playback polling is the existing Phase 2 cadence (5 s playing, 30 s idle). In the background it stops, except while the last confirmed active Connect **device id** is the local librespot device, when that same 30 s poll continues. This is not a new Now Playing poll.
...
- librespot is launched under a stdin-pipe wrapper so the child dies when the parent exits (crash, SIGKILL, Xcode stop). Stale `--name SpotifyLite` processes for the same `--system-cache` are reaped on the next `start()`. The credential cache is not deleted. `start()` is single-flight so concurrent callers never spawn a second process.
- librespot starts only after **explicit per-account opt-in** (“This Mac (set up…)”, then “Play on this Mac” / Retry: `playOnThisMac` → consent gate → `ensureLocalDevice` → `start()`). It does **not** launch to recover from a missing device (404 / no active device), at sign-in, or on locator/discovery success. Remote control stays the default. Crash restart runs only for that user-started session; after max attempts, degrade to remote + banner and **stop** launching. Its wrapper waits for process/pipe events without a polling loop. Now Playing uses this same 5 s / 30 s playback poll and exact device id; there is no extra Now Playing poll.

...
- librespot starts only from the existing #16 path: **Play on this Mac** / Retry (`playOnThisMac` → inline consent gate → `ensureLocalDevice` → `start()`). Until #16's store (PR #29) merges, that gate defaults true — merge order, **not** a 404 auto-start. `playContext` does **not** launch on 404 / no-device, sign-in, or locator/discovery success. Crash restart runs only for that user-started session; after max attempts, degrade to remote + banner and **stop** launching. Its wrapper waits for process/pipe events without a polling loop. Now Playing uses this same 5 s / 30 s playback poll and exact device id; there is no extra Now Playing poll.
